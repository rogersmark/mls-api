...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for rt, points in RESULT_TYPES:
            orm['mls_api.Result'].objects.get_or_create(
                name=rt.capitalize(),
                code=rt,
                points=points
            )

        win = orm['mls_api.Result'].objects.get(code='win')
        draw = orm['mls_api.Result'].objects.get(code='draw')
        loss = orm['mls_api.Result'].objects.get(code='loss')
        for game in orm['mls_api.Game'].objects.all():
            home_team = game.gameteam_set.get(home=True)
            away_team = game.gameteam_set.get(home=False)
            if game.home_score > game.away_score:
                ...
                home_team.result = draw
                away_team.result = draw
